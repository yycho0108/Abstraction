...
function pNoise(grid,x,y){
    ...
    var v1 = v2dot( grid(x_bot,y_bot), [x-x_bot,y-y_bot]); // LD
    var v2 = v2dot(grid(x_bot+1,y_bot), [x-(x_bot+1),y-y_bot]); //RD
    var v3 = v2dot(grid(x_bot,y_bot+1),[x-x_bot,y-(y_bot+1)]); //LU
    var v4 = v2dot(grid(x_bot+1,y_bot+1),[x-(x_bot+1),y-(y_bot+1)]); //RU
    var tx = x-x_bot;
    var ty = y-y_bot;
